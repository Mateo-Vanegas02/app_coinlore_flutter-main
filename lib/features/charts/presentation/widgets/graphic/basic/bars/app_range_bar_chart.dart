...
class GraphicRangeBarChart extends StatelessWidget {
  final List<Map<String, dynamic>> data;
  final bool showAxes;

  const GraphicRangeBarChart({
    super.key,
    required this.data,
    this.showAxes = true,
  });
  ...
  Widget build(BuildContext context) {
    ...
    for (final item in data) {
      final min = ((item['min'] ?? item['low'] ?? 0) as num);
      final max = ((item['max'] ?? item['high'] ?? 0) as num);
    ...
    return Chart<Map<String, dynamic>>(
      data: data,
      variables: {
        'crypto': Variable(accessor: (Map m) => (m['crypto'] ?? m['asset'] ?? m['x'] ?? '') as String),
        'min': Variable(accessor: (Map m) => ((m['min'] ?? m['low'] ?? 0) as num), scale: sharedScale),
        'max': Variable(accessor: (Map m) => ((m['max'] ?? m['high'] ?? 0) as num), scale: sharedScale),
      },
